...
class UserPhotoAPITestCase(TestCase):
    ...
    def test_create(self):
        ...
        tmpim = make_image()
        with open(tmpim.name, 'rb') as data:
            response = self.client.post(reverse('profile-photos', kwargs={'pk': self.p1.pk}), {
                'image': data,
    ...
    def test_retrieve(self):
        ...
        response = self.client.get(reverse("profile-photo", kwargs={"pk": self.photo1.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update(self):
        ...
        response = self.client.patch(
            reverse("profile-photo", kwargs={"pk": self.photo1.id}), data={"description": desc}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.patch(reverse("profile-photo", kwargs={"pk": self.photo2.id}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_delete(self):
        """Tests that a user can delete their own photos but not others"""
        response = self.client.delete(reverse("profile-photo", kwargs={"pk": self.photo2.id}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        response = self.client.delete(reverse("profile-photo", kwargs={"pk": self.photo1.id}))
